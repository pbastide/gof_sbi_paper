...
#
# It saves all the simulated data in the "diyabc_reftables/gaussianlaplace" repository.
...
dir.create(here("diyabc_reftables", "gaussianlaplace"))
...
for (lmomname in lmomnameall) {
  for (refname in refnameall) {
    for (testname in refnameall) {
      ...
      pdf(file = here("diyabc_reftables", "gaussianlaplace", paste0(results_name, "_pca.pdf")))
      ...
      saveRDS(list(data.ref = data.ref,
              ...
              file =  here("diyabc_reftables", "gaussianlaplace", paste0(results_name, "_data.rds")))
    }
  }
}
