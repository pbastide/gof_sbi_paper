...
#
# REQUIREMENTS:
# - File "headerRF_for_diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen.txt"
# - File "diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen.bin"
#   Generated by script "04_Generate_reftableRF.bin_for_diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen.sh".

################
# Cleaning of all objects from R memory
rm(list=ls())
################
library(abcrf)
options(max.print=100000)

#### FOUR case studies can be processed in the present script
human.fast.case = FALSE
depindep.case = FALSE
human.real.dataobs.for.PREinference.GoF.case = FALSE
human.real.dataobs.for.POSTinference.GoF.case = TRUE
...
if (human.real.dataobs.for.PREinference.GoF.case == TRUE) {
  file.reference.sim <- "diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_allS_11000PerScen.bin"
  file.header <- "headerRF_for_diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_allS_11000PerScen.txt"
  output.file.prefix <- "human_12000snp_maf_hudson_reftableRF_allS_11000PerScen"
}
if (human.real.dataobs.for.POSTinference.GoF.case == TRUE) {
  file.reference.sim <- "diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen.bin"
  file.header <- "headerRF_for_diyabc_generated_reftableRF_human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen.txt"
  output.file.prefix <- "human_12000snp_maf_hudson_reftableRF_S2S3_110000PerScen"
}

# Importation of the reference table and observed data
#nparticules.reftable <- 220000
...
# Special instruction when, for simulation efficiency, the reftableRF.bin includes only two human scenarios 1 and 2
# which actually corresponds to the scenarios 2 and 3 of the original header containing 6 human scenarios
if (human.real.dataobs.for.POSTinference.GoF.case == TRUE) {
  index_scen <- as.numeric(as.character(index_scen)) + 1
  N.scen <- reftable$nscen+1 # the number of scenarios taking into account the above trick
}
...
############################################################################################################################################################################################
  ...
  if (human.fast.case == TRUE || human.real.dataobs.for.PREinference.GoF.case == TRUE || human.real.dataobs.for.POSTinference.GoF.case == TRUE) {
