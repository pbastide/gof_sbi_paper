...
#   - To be plotted by script `gof_simulaiton_study_plots.R`
...
for (dataname in c("gaussianlaplace", "depindep", "human")) {
  ...
  for (test_in_alternative in c(TRUE, FALSE)) {
    ...
    if (dataname == "depindep") {
      ...
      source("diyabc_simulation_scripts/sim_depindep.R")
      path_to_headers <- here("diyabc_simulation_scripts", "deptindep_resim_nocond")
      seed <- 1289
      ncores_sim <- ncores
    ...
    if (dataname == "human") {
      ...
      ## Simulation function used for the reference table (for post-inference GoF tests)
      # Source simulation function
      source("diyabc_simulation_scripts/sim_human.R")
      path_to_headers <- here("diyabc_simulation_scripts", "human_resim")
      seed <- 1289
      ncores_sim <- ncores
      datestamp_simu <- datestamp_day
