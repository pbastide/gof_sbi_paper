...
#
# REQUIREMENTS:
#   - DIYABC: version 1.1.51 or above. https://github.com/diyabc/diyabc/releases.
...
#
# DATA
#   Data must be at "diyabc_reftables/humanreal".
#
# NEW POINT SIMULATIONS:
# The post-inference tests require the simulation of new particles from the reference model.
# Requirements:
#   - Simulation script: "diyabc_resimulation_scripts/sim_humanreal.R"
#   - Reference tables: "diyabc_resimulation_scripts/humanreal_resim"
#   - Working installation of DIYABC
#
# OUTPUTS:
...
reffolder <- "diyabc_reftables/humanreal"
datafolder <- "human_data"
################################################################################
...
data.obs <- as.matrix(read.table(file.path(datafolder,file.dataobs),header=TRUE), drop = FALSE)
data.obs.replica <- as.matrix(read.table(file.path(datafolder,file.dataobs.replica),header=TRUE), drop = FALSE)
...
param.ref.all <- as.matrix(read.table(file.path(reffolder, posterior.gof.file.param), header = TRUE))[1:100000, -1]
#############################################################################################
...
if (GOF.PRIOR==TRUE) {
  ...
  for (i.scen in 1:n.scenario.prior.gof) {
    ...
    data.ref <- as.matrix(read.table(file.path(reffolder, prior.gof.file.ref.i.scen), header = FALSE))[1:10000, 2:131]
    #data.ref.replica <- as.matrix(read.table(file.path(reffolder, prior.gof.file.ref.i.scen.replica), header = FALSE))[1:10000, 2:131]
...
if (GOF.POSTERIOR ==TRUE) {
  ...
  data.ref <- as.matrix(read.table(file.path(reffolder, posterior.gof.file.ref), header = FALSE))[1:100000, 2:131]
  #data.ref.replica <- as.matrix(read.table(file.path(reffolder, posterior.gof.file.ref.i.scen.replica), header = FALSE))[1:10000, 2:131]
  colnames(data.ref) = colnames(data.obs)
  #colnames(data.ref.replica) = colnames(data.obs)
  param.ref.all <- as.matrix(read.table(file.path(reffolder, posterior.gof.file.param.i.scen), header = TRUE))[1:100000, -1]
  ...
  source(here("diyabc_resimulation_scripts/sim_humanreal.R"))
  path_to_headers <- here("diyabc_resimulation_scripts", "humanreal_resim")
